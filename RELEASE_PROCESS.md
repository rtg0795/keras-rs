...
⚠️ This doc is intended for maintainers of the KerasRS library. Steps below
...
- Adding a new release to the [keras-rs](https://pypi.org/project/keras-rs/)
...
1. Similar to the Keras and Tensorflow repositories, we keep a named branch
   ...
   `upstream` as `keras-team/keras-rs` by running:
   
   ```shell
   git remote add upstream https://github.com/keras-team/keras-rs.git
   ```
...
2. Before we officially push a new stable release to pypi, it is good practice to
   ...
   On the relase branch, check the version in `keras_rs/src/version.py`. If the
   current version is not `X.Y.0.dev0`, make a PR following [this template](<place_holder>)
   ...
   # Update keras_rs/src/version.py with an editor.
...
3. On github, we can now create the `X.Y.0.dev0` release. Use
   [this link](https://github.com/keras-team/keras-rs/releases/new) to kick it
   off.
   ...
   ![Release page screenshot](<place_holder>)
...
4. Wait a few minutes until the release appears on pypi, then test out the
   release by running `pip install keras-rs==X.Y.0.dev0`.
...
5. Before cutting the final release, we should try previewing our documentation
   ...
   During development of the branch, you can pin the keras-rs dev release in
...
2. Before we officially push a new stable release to pypi, it is good practice to
   ...
   On the relase branch, check the version in `keras_rs/src/version.py`. If the
   current version is not `X.Y.Z.dev0`, make a PR following [this template](<place_holder>)
   ...
   # Update both keras_rs/src/version.py with an editor.
...
3. On github, we can now create the `X.Y.Z.dev0` release. Use
   [this link](https://github.com/keras-team/keras-rs/releases/new).
...
4. Wait a few minutes until the release appears on pypi, then test out the
   release by running `pip install keras-rs==X.Y.Z.dev0`.
